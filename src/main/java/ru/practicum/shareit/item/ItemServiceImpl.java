...
public class ItemServiceImpl implements ItemService {
    ...
    @Autowired
    public ItemServiceImpl(ItemRepo repo, UserRepository userRepo) {
        this.repo = repo;
    ...
    public ItemDto add(ItemDto item, Long ownerId) {
        return repo.add(item, userRepo.get(ownerId)).orElseThrow();
    }

    @Override
    public ItemDto patch(ItemDto item, Long id, long ownerId) {
        return repo.patch(item,id, userRepo.get(ownerId)).orElseThrow();
    }
